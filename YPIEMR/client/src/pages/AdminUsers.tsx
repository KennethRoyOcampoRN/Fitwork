              ...
              {users.map((u) => (
                <tr key={u.id}>
                  ...
                  <td className="p-2 space-x-2 whitespace-nowrap">
                    <button onClick={() => toggleActive(u)} className="text-xs text-clinic-700 underline">{u.isActive ? "Deactivate" : "Reactivate"}</button>
                    <button onClick={() => resetPassword(u)} className="text-xs text-clinic-700 underline">Reset password</button>
                  </td>
                </tr>
              ))}
            </tbody>
